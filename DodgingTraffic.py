...
def draw_text(surface, text, color, font_size, x, y):
    ...
    total_width = sum(font.size(word + " ")[0] for word in words if word not in color_map)
    ...
    for word in words:
        if word in color_map:
            ...
            text_surface = font.render(word + " ", True, current_color)
            ...
            if event.type == QUIT:
                ...
                if event.type == pygame.JOYHATMOTION:
                    ...
                    if event.button == 0:  # A button
                        ...
                        return text
                    elif event.button == 2:  # X button -> BACK
                        text = text[:-1]
                    elif event.button == 3:  # Y button -> ENTER
                        return text
                    elif event.button == 2:  
                        text = text[:-1]
...
def main_game_loop():
    global running, game_over, is_paused, player_name, player_score, score_saved, selected_backstory, car_counter, ramp_up, rect_creation_interval, last_rect_creation_time, rectangles, point_increase_timer, score_increment_period, difficulty
    title_screen_display()
    ...
    while running:
            ...
            if event.type == pygame.QUIT:
                ...
                if js:
                    if event.button == 0:  # A button
                        ...
                        # Bring up pause menu instead of game over
                        if not game_over:
                            is_paused = True
                        else:
                            game_over = True
        ...
        if game_over:
            ...
            # Display difficulty in the top right
            difficulty_names = ["Easy", "Medium", "Rush Hour", "..."]
            current_difficulty = difficulty_names[difficulty] if difficulty < len(difficulty_names) else f"Difficulty {difficulty}"
            draw_text(win, f"{current_difficulty}", (200, 200, 200), 24, width - 180, 10)
